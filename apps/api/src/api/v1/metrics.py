...
@router.get("/weekly")
async def weekly_summary(session: AsyncSession = Depends(get_session)):
    ...
    q_total = await session.execute(
        select(func.count()).select_from(Interview).where(Interview.created_at >= week_ago_naive)
    )
    q_completed = await session.execute(
        select(func.count()).select_from(Interview).where(Interview.completed_at >= week_ago_aware)
    )
    return {
